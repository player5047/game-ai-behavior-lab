...
TILEMAP_FILE = os.path.join(app.static_folder, "assets", "village", "tilemap", "tilemap.json")
DEFAULT_TILEMAP_SIZE = {
    "tile_width": 32,
    "map_tile_width": 29,
    "map_tile_height": 39,
}
...
def load_tilemap_size():
    try:
        with open(TILEMAP_FILE, "r", encoding="utf-8") as f:
            tilemap = json.load(f)
        return {
            "tile_width": int(tilemap.get("tilewidth", DEFAULT_TILEMAP_SIZE["tile_width"])),
            "map_tile_width": int(tilemap.get("width", DEFAULT_TILEMAP_SIZE["map_tile_width"])),
            "map_tile_height": int(tilemap.get("height", DEFAULT_TILEMAP_SIZE["map_tile_height"])),
        }
...
def index():
    ...
    zoom = float(request.args.get("zoom", 0))    # 画面缩放比例，0 表示自动适配整张地图
